...
**2026-09-23 Chrome 仅勘误验证步骤，未采纳或冻结本稿设计。** 到货第一轮以 `docs/MEASUREMENT_PROTOCOL.md` 第 0～5 节的认版、外观和安全外表面测量为界。下列 `Q07/Q08` 三视图、标尺和孔位标注**依赖待审 [PR #55](https://github.com/ChromeTokyo/MosaicoKeyboard/pull/55) 的 T14 v0.3 规程/记录表；本勘误不能先于 #55 当作完整到货指引**。下表后续夹具、试插、上电、通断和 G6 项目是待制定/复核的验证计划，**不是用户到货即可执行的动作**。关机和拔 USB 不等于内置电池已隔离；没有同版针位图、可靠夹具及 Hiro 复核时保持 `OPEN`，照片估算不冒充制造尺寸。
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| AS-01 | Mosaico 整机外形 45.19 × 45.19 × 11.48 mm，33 g（官方视频简介标称，非图纸） | ICD 第 1、5 节；mech-module、mech-dock OpenSCAD 顶部块 | 托架内腔、模块板长度、握把总长全部错；可能夹不住或晃动 | 按 T14 第 3 节：绝缘外壳外表面 W/H 各量 3 次；T 仅当前后都可安全接触绝缘边框时量，否则只拍侧视并记未测。突出物另记；质量待有合适电子秤时另称，未称不关闭本项 | 卡尺（用户已有）、手机；电子秤另备 | 用户测、Chrome 回填、Hiro 复核 | OPEN |
...
| AS-03 | 母座位于机身 −X 面（屏朝用户、USB-C 朝下时的左侧），开口朝 −X，与外壳面近平齐 | ICD 第 1、5 节；mech-module | 模块板转向件几何全错 | 第一轮保留 T14 先贴的 T↑ 原标签；另拍屏朝人/USB-C 朝下姿态的 Q07 正视、斜视、定位照，供 Chrome 与 ICD 轴映射，不重贴 T↑。卡尺只量绝缘外壳外表面；开口凹/凸深度无同平面侧向参照或专门绝缘检具时记“未测” | 手机、标尺；后续绝缘检具待设计 | 用户拍照、Chrome 回算 | OPEN |
...
| AS-22 | 原生 USB-C 位于 −Y 面、不被托架与模块板遮挡，双 USB 可同时插 | ICD ME-D-07；mech-dock | 无法双供电或无法用原生口调试 | 第一轮按 T14 M06/P14 拍 USB-C 与邻近外壳、旁置标尺，记录开口和插头操作空间；卡尺只量绝缘外壳外表面，不把量爪伸入小开孔。接口中心 X/Z 与双 USB 可用性待后续几何回算、首版托架和无源插头试配 | 手机、标尺；后续托架与无源插头 | 用户、Chrome | OPEN |
