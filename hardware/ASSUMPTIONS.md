...
| AS-02 | 左槽 H2 为 2×10P、间距 2.54 mm | ICD 第 2、5 节；module-board 公头封装 | 模块板公头插不进或错位；整块模块板重打 | 卡尺量相邻针中心距 3 处；量 pin 1 到 pin 19 中心跨距（应≈ 9 × 2.54 = 22.86 mm）；照片 + 比例尺 | 卡尺、手机、2.54 mm 排针作比例尺 | 用户、Chrome | OPEN |
| AS-03 | 母座位于机身 −X 面（屏朝用户、USB-C 朝下时的左侧），开口朝 −X，与外壳面近平齐 | ICD 第 1、5 节；mech-module | 模块板转向件几何全错 | 基准姿态下拍 −X 面正视照；量母座开口平面相对外壳面的凹/凸深度 | 手机、卡尺/深度尺 | 用户 | OPEN |
| AS-04 | 槽中心 Z 高度约为厚度中点；Y 位置 unknown，以参数 `SLOT_PIN1_Y`、`SLOT_PIN1_Z` 表示 | ICD 第 1 节参数；mech-module | 模块板公头位置错、触点面高度错 | 量 pin 1 中心到 −Y 面、到 +Z 面距离；量 pin 20 同样两值；记录 10 针方向沿 Y 或 Z | 卡尺 | 用户 | OPEN |
| AS-05 | 槽连接器为轴向插接排针/排母（E-01 未证实）；四角银色块为整机之间组合用磁铁，非模块电气连接 | ICD 第 5 节 ME-D-05；方案可行性前提 | 若为磁吸侧向触点，方案 D＋G 的公头方案不成立，需重定接口 | 目视母座孔形；用回形针试四角是否吸附；**不带电**用绝缘塑料探针轻试插入深度；不拆机 | 回形针、塑料探针 | 用户、Chrome 判读 | OPEN |
| AS-06 | `B-2200R20P-B120`（V1.0 原理图标注）的配对件是 2.54 mm 2×10 排针类公头；候选料「待原厂规格核对」 | ICD ME-D-06；module-board BOM | 插拔力、针长、保持力不匹配；可能损伤母座 | 取得该母座原厂规格书，核针径、配合针长范围、插拔力；用候选公头**只试插一次**并记录手感与是否到位 | 原厂规格书、候选公头样品 | Chrome | OPEN |
...
| AS-17 | H2 针号与物理位置：奇数针行位置、pin 1 朝向与 D1 表一致 | ICD 第 1、2 节；module-board 焊盘序 | 全部 20 针镜像错，pin 17 5 V 可能落到 GPIO | 到货后 Mosaico **关机、不接任何外设**，万用表二极管档/通断档：pin 20 对已知 GND（USB-C 外壳）通；pin 19 对 3.3 V 网络的特征；拍照标注 pin 1；不得带电探 | 万用表（用户已有）、细探针 | 用户测、Chrome 判读 | OPEN |
...
| AS-24 | 左槽无锁扣，模块板保持力不足以承受每天插拔底座的力；需机械辅助固定到 Mosaico 本体 | ICD ME-D-08；mech-module | 模块板在槽内松动，H2 接触不良 | AS-05/AS-06 试插后手感；首版机械件装上后做 G6-G 插拔 | 样件 | mech 任务、用户 | OPEN |
