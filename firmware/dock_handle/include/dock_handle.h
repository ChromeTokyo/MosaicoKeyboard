 ...
 *   2. 用 bsp_subboard_map_gpio() 取 KEY_* 对应 GPIO，配置为输入＋内部上拉；
 *   3. 20–40 ms 周期轮询＋N 次一致去抖（BSP 无中断通路，见 LEFT_SLOT.md）；
 *   4. 以 FreeRTOS 队列投递 dock_handle_event_t；提供按键位图；
 *   5. 监听管理器事件，模块拔出时释放 GPIO 与 lease；可选自动重新认领。
 *
 ...
 *   _slot_to_name / _type_to_name（mosaico_module_mgr.h）；
 *   bsp_subboard_map_gpio（bsp/subboard.h）。
 */
...
typedef enum {
    ...
    DOCK_HANDLE_EVENT_ATTACHED, /*!< 已认领槽位并配置好全部 KEY_* GPIO */
    DOCK_HANDLE_EVENT_DETACHED, /*!< 模块被拔出或驱动停止；lease 已释放，之前按下的键已补发释放事件 */
} dock_handle_event_type_t;

/** 按键事件。任务要求的 {key, pressed, timestamp} 之外增加 type 以承载热插拔通知。 */
typedef struct {
...
typedef struct {
    ...
    uint32_t claim_timeout_ms;      /*!< init 阻塞等待首次认领的时间；0 = 不等待（未立即认领则返回 ESP_ERR_TIMEOUT，auto_reattach 时后台继续） */
    ...
    bool check_identity;            /*!< 认领前核对 EEPROM vendor_id / board_id */
    uint16_t vendor_id;             /*!< check_identity 时要求的 vendor_id */
    uint16_t board_id;              /*!< check_identity 时要求的 board_id */
 ...
 *
 * 驱动是单例。内部调用 mosaico_module_mgr_init(NULL)，与其他模块驱动一样可重复调用。
 *
