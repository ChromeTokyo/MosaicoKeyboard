...
| D-PWR-USB | 待审底座电源提案仅有 Type-C Rd，却固定充电/系统输入限约 1.46 A；未按源广告能力设限。见[D3复核](../review/chrome/D3-power/TOPOLOGY_REVIEW.md)。 | 以硬件判 CC 能力并切换输入限流，或按可接入源最弱档保守设限；重做 USB-only 无电池与热预算，非作者复核。 |
| D-PWR-FB | 待审 TPS61023 反馈电阻原909k/120k算得5.102V，但建议替换768k/750k分别实际仅4.403V/4.314V；pin17输入窗口未知。 | 主控修 `POWER_TOPOLOGY.md` 与 `POWER_BUDGET.md` 同源数字，重新复算 min/typ/max；获取V1.2 pin17适用额定后方可定电压与保护阈值。 |
| D-G2-GATE | 待审sweep的形状检查在15/15 n/a时仍EXIT0，跨分支检查缺22/23必需关系时仍EXIT0；与G2实际碰撞/超时结论不同，前者是脚本本身的假绿漏洞。 | 机器闸门须对缺被测模型、编译失败、超时与缺必需键返回非零；用替身OpenSCAD回归后再跑真实同SHA模型，Hiro/Grok非作者复核。实际G2的2碰撞＋1超时由机械源作者另修。 |
