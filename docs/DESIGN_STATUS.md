...
## Chrome 资料复核补记：V1.2 输入与 J1（2026-09-23）

以下只更新开放问题，不覆盖上面的历史结论或替主控冻结接口。证据与准确边界见 [V1.2 输入报告](../review/chrome/T20-takeover/V12_INPUT_BOUNDARY.md)和[J1 配对候选](../review/chrome/T20-takeover/J1_MATING_OPTIONS.md)。

| ID | 新证据与仍未知 | 影响与关闭条件 |
| --- | --- | --- |
| D-IF-01 补记 | 乐鑫现行产品页 `IN` 为外部 5 V 供电；同页 100 mA 只限 `5V`/`3V` 输出。旧 CoreBoard V1.0 指南所说 `VIN` 可充电，**不能证明目标 V1.2 经 H2.17 会充电**；V1.2 输入额定、合路/反灌仍 unknown | 取得目标 BaseBoard/CoreBoard 电气图及厂家额定，或到货限流验证指定测试上限；未闭合前不冻结 pin17 供电预算与 G3 |
| D-IF-04 | 模块板提案的 C9144 带罩公头与项目旧假设中的 C124406 母座未见厂家交叉配对保证；旧假设称其见于 V1.0 原理图，但本轮未在公开 V1.0 用户指南文字或已归档 CoreBoard 图直接检到料号；目标 V1.2 母座料号 unknown；CJT C239341/XFCN C492438 仅为可购样的裸弯公头候选 | 到货核母座料号、断电试插与 20 针通断；改 J1 料须重建封装/净空和尺寸链、重跑 G2，非作者复核后才选型 |
| D-IF-05 | 官方归档扩展指南 p2 限 **1.2.1 及以上**，视频所见丝印 V1.2 与其适用关系 unknown | 原厂版次对照或到货实物与指南步骤/结构逐项核对；不得从文件名 `official-v12` 推定适用 |
