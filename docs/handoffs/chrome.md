...
### 0.5 两份 `docs/handoffs/chrome.md` 的归属与合并

| 项目 | 约定 |
| --- | --- |
| 文件归属 | `docs/handoffs/chrome.md` 是 **Chrome 的交接文件**，长期归属 Chrome。主控只在首次派发时写入工作包内容 |
| 保留哪一份的字段结构 | 保留 `docs/handoffs/TEMPLATE.md` 的字段结构。远端 Chrome 版已按该模板组织（抬头 + 已完成 + 文件与事实依据 + 未解决与下一步），主控版的任务派发内容属附加章节 |
| 谁并入谁 | 以 **Chrome 分支上的版本为基础**，把本包中 Chrome 尚未覆盖的内容（T03–T05 派发、第 0.4 节路径核对、第 5 节接口文件约定、第 6 节盘点线索、第 7 节登记与进展日志要求）作为附加章节并入。不要反向用主控版覆盖 Chrome 已提交的 T02 记录 |
| 合并执行人 | Chrome。主控不在 Chrome 的分支上直接改写该文件 |
| 未决 | 主控本轮改动尚未提交（第 0.3 节），合并时点取决于第 0.4 节末段选甲还是选乙。在主控提交前，Chrome 可先按本文件内容工作，不必等待 |
