...
- 更正**成立，不是过度反应**。理由：其逻辑不依赖范围 (a) 的精确置信度。即便背面仍有焊盘，模块槽也是官方专门为扩展准备的接口，V1.2 还为它配了独立 I²C 总线；走模块槽使 MCU 与 I²C 扩展器**成为可省的**。**（2026-09-21 按 Hiro H0 F05 整改：原文为「不需要 MCU，也不需要 I²C 扩展器……MCU 就失去存在理由」，把「可省」写成了「必须省」。）**准确表述是：方案 H 引入 MCU 的**主要**动机是绕开「背面无 I²C」，该动机消失后 MCU 不再是必需项；但 GPIO 数量足够并不证明 MCU 或 I²C 扩展器必然无价值（固件成本、对接触点数量、扩展余量等仍会影响取舍），UD-B 是否撤销已授权的 MCU 选项仍由用户决定。
...
| c4 | V1.2 模块槽为独立 `I2C_NUM_1`（GPIO0／1），主板改 GPIO56／3；主板地址 `0x11 0x12 0x19 0x55 0x5A 0x69` 不再占用模块总线；**（2026-09-21 按 Hiro H0 F03／§5 c4 整改：原文写「模块总线上只有 `0x50`／`0x51`」，这是派发包的转述错误，不是 Chrome 的结论——Chrome `LEFT_SLOT.md:38` 原文已写「其他模块地址仍须管理」，比本包转述更窄且正确。）**准确表述为：**已注册的 EEPROM 地址为 `0x50`／`0x51`，其他模块的器件地址仍须管理**——左右两槽共享这条模块总线，任何模块自带的器件都会占用地址空间 | `esp_mosaico.h:62–75`、`subboard.c:73–98` | 读源码确认；注意 V1.2 分支开启的是**内部上拉**，外部上拉是否存在、是否足够为 `unknown`；判断 `docs/DECISIONS.md` D-010（V1.0 CoreBoard R1／R2 4.7 kΩ）的适用范围是否应显式限定为 V1.0 共享 I2C0 |
...
| c7 | 硬约束二：EEPROM 从 pin19 `VCC_3V3` 取电，**不得从底座反送 3.3 V**；pin18 `5V_OUT` 留空不接 | `user_guide_v10.rst:654–662`（pin19 `VCC_3V3` 与 pin18 `5V_OUT` 均「controlled by GPIO60」）；`DOCUMENT_BOUNDARIES.md`（V1.0 原理图有 D14 旁路，GPIO60 不是无电保证） | 判断反灌风险表述是否成立；确认 D14 旁路结论仅对 V1.0。**（2026-09-21 按 Hiro H0 F05 整改：原文写的「EEPROM 在主机未调用 `bsp_subboard_init()` 时无电、模块不会被识别」是错的，本包据此向 Hiro 提问也是错的。）**实际情况：`onboard/esp_mosaico.c:143–178` 会**保留前级已设置的电源状态**，`:241–261` 另有单独开启电源的函数，因此「未调用 `bsp_subboard_init()`」推不出「pin19 rail 必然无电」。要写进合同的是电源时序本身的约束，而不是这条被证伪的推论 |
...
| c10 | 软件面：**（2026-09-21 按 Hiro H0 F05 整改，原文为「250 ms 轮询 ＋ 3 次去抖，无中断通路；空白 EEPROM 不合格」，两句都过绝对）**① **模块管理器的发现机制**采用 250 ms 轮询 ＋ 连续 3 次 presence 去抖；这只说明**发现机制**是轮询，**推不出 GPIO「无任何中断通路」**——ESP-IDF 提供 `gpio_set_intr_type()` 等中断 API，本项目的**按键读取方式尚未冻结**，轮询与中断都还在可选范围内。② 空白 EEPROM 的行为要分两句说：它**可以 ACK、会被判为 PRESENT**；但其描述为 INVALID，**通不过正常的 claim 流程**——不是「必然探测不到」。③ 烧入合法身份不会自动产生十键驱动 | `mosaico_module_mgr.h:29–33`；`mosaico_module_mgr.c:458–600, 994–1047`；`mosaico_module_interact/mosaico_module_interact.c:785–866` | 抽查行号与结论是否对应；这一项影响固件工作量，不影响电气合同 |
...
- c6–c8 三条硬约束主控认为成立，且必须写进后续 ICD 改版。主控另建议补两条供 Hiro 判断是否列入合同：（i）底座 USB-C 与 Mosaico 原生 USB-C 可能同时供电，底座对 pin17 的输出必须防反灌，且防反灌不得依赖任何软件判断；（ii）pin19 `VCC_3V3` 受 GPIO60 控制、由 `bsp_subboard_init()` 主动开启。**（2026-09-21 按 Hiro H0 F05 整改：原文后半句「若主机固件未调用该初始化则 EEPROM 无电，模块不会被识别」是错的，已删去该推论。）**`onboard/esp_mosaico.c:143–178` 会保留前级已设置的电源状态，`:241–261` 另有单独开启电源的函数，故「未调用初始化」不等于「rail 无电」；要写进合同的是 pin19 供电由主机侧控制这一事实本身，以及模块侧不得假定该 rail 何时带电。
...
| 编号 | 假设 | 目前依据 | 到货后验证步骤 |
| --- | --- | --- | --- |
| A-H0-1 | `ASSUMPTION:` V1.2 左槽 20 针的电气定义与官方 V1.0 用户指南 H2 表逐脚一致 | 实拍 `v12-baseboard-back.png` 与官方线图上左槽阵列的丝印集合与 H2 表内容一致（主控本次目视：`55 ADC`、`33 DN`、`34 DP`、`IN`、`3V3`、`14`、`1 SCL`、`0 SDA`、`5V`、`GND` 等可辨，顺序与朝向未逐脚核对）；BSP 左槽映射函数对左槽原样返回 | 按 `docs/MEASUREMENT_PROTOCOL.md` 第 4B.3 节拍左槽特写并逐脚抄丝印。**（2026-09-21 按 Hiro H0 F01 整改：原文只写「整机断电、隔离电池后…做通断」，未逐网列清判据，会把 pin19 的 3.3 V 与背面 5 V 相通误当成验收通过。）**通断测量前先按 A-H0-4 ① 完成断电与电池隔离并确认无残余电压；被测网络与判据**逐网列清**，不同电压的网络相通不构成通过：① pin20 `GND` ↔ 背面 `GND` 焊盘，预期导通；② pin17 `5V_IN` ↔ 背面 `5V` 焊盘，两者是否同网为 `unknown`（中间可能有二极管或开关），实测结果只作登记，**导通与否都不构成本条的通过判据**；③ pin19 `VCC_3V3` ↔ 背面 `5V` 焊盘，**分属 3.3 V 与 5 V 两个网络，预期不导通；若测得导通是异常，须停下查明，绝不得当作验收标准**；④ pin14／pin16 ↔ 背面 2×10P 丝印 `1`／`0`，核对是否落在同一 GPIO 网络上，逐脚记录。以上均为在路测量，半导体通路会给出方向相关读数，须正反两个极性各测一次并记录两个读数 |
...
| A-H0-3 | `ASSUMPTION:` eFuse 中的 board variant 与 BaseBoard 丝印 `V1.2` 严格对应 | `esp_mosaico.h:42–43` 从 eFuse USER_DATA 读；variant 烧在 CoreBoard 芯片内、丝印在 BaseBoard 上。**（2026-09-21 Hiro 补查 `onboard/esp_mosaico.c` 指出：eFuse 1.1 与 1.2 都映射到 `variant V1_2`，因此 variant 只能区分 V1_0／V1_2 两档，不能分辨 1.2 与指南所称 1.2.1，也不能证明 CoreBoard 与 BaseBoard 同版。）** | 刷任意 BSP 示例，抄录串口日志中的板级版本打印；同时按第 0 节抄录 BaseBoard 与 CoreBoard 丝印。**（2026-09-21 按 Hiro H0 F02 整改）采集时必须分别保存两个字段：日志中的原始 `vX.Y` 与映射后的 `variant`，二者不是同一字段**——eFuse 1.1 与 1.2 同映射 `V1_2`，映射值会丢失原始版号，只登记 `variant` 等于丢掉证据。原始 `vX.Y`、映射 `variant`、BaseBoard 丝印三值并列登记，**本样本的观察与全版次结论分开写**；在官方给出版本对照或 BaseBoard 资料前，variant／BaseBoard／指南三者的映射关系保持 `unknown` |
| A-H0-4 | `ASSUMPTION:` V1.2 模块槽 I²C1 外部上拉存在与否及阻值未知；按「可能只有内部上拉」设计并预留 DNP 位 | `subboard.c:93` `enable_internal_pullup = true`；V1.0 的 4.7 kΩ 结论不适用 | **（2026-09-21 已更正，原文为「主机上电…测…电阻」，属带电测电阻的错误做法，由 Hiro 在 H0 复核中指出）****（2026-09-21 按 Hiro H0 F01 整改：上一版新句「读数即外部上拉的存在与阻值」仍过度——在路读数含并联路径与半导体通路；「完全断电」也未写明电池隔离。）** 分三步且**严禁带电测电阻**：① **断电与隔离**：关机后断开所有 USB，并**隔离电池**（拔电池接插件或按官方流程断开电池连接；**仅关机不等于断电，关机不可代替电池隔离**）；用电压档确认被测点对 GND 无残余电压，对可能的储能元件放电后再确认一次为 0 V，才可切到电阻档。② **在路电阻测量及其读数边界**：用万用表电阻档测 pin14／pin16 对 pin19 的直流电阻。**这是在路（in-circuit）测量，读数是两点之间全部并联路径的合成值**，其中可能包含 CoreBoard 内部上拉、其他分支电阻，以及 ESD／钳位二极管等半导体通路（后者会使读数随表笔极性变化）。因此**读数不等于独立外部上拉的阻值，也不能仅凭一个读数断定外部上拉存在或不存在**：正反两个极性各测一次并记录两个读数；只有读数稳定、双向一致且量级与常见上拉（数 kΩ）相符时，才可登记为「疑似存在外部上拉」的**线索**，阻值仍标 `unknown`，最终判定须结合 V1.2 原理图或拆件测量。③ **上电只做电压／波形测量，不测电阻**：测 pin14／pin16 空闲时对 pin20（GND）的电压，应接近 pin19 电平；有示波器再看 100 kHz 读 EEPROM 时的上升沿，与总线电容一并推断上拉强度是否足够。三项结果合并后决定 DNP 位是否装配；在上述测量给出结论前，本条的 `unknown` 不解除 |
| A-H0-5 | `ASSUMPTION:` pin17 输入额定未知；电源预算按「整机峰值 ≤ 1 A、持续 ≤ 0.6 A」假设计算，无官方依据 | 官方仅写「可供电并充电」，无数值（`DOCUMENT_BOUNDARIES.md`） | **（2026-09-21 按 Hiro H0 F08 整改：原文「从 0.3 A 逐级上调……任何异常即停」会把预算和试探当成额定，已改写。）**① 本条的 1 A／0.6 A **只是电源预算场景**，`ASSUMPTION:` 标签不解除，**不得被当作 pin17 的安全输入额定**；同理 **0.3 A 不是有据的安全起点**，官方未给任何数值。② **删除「从 0.3 A 逐级上调到异常即停」的做法**——逐级加到出现异常只能记录一次失效，**不能建立额定**，还可能在建立额定之前先损坏样机。③ 正确顺序是**先立上限、再测**：由 Chrome 后继者按已核对的供电路径与器件约束（V1.2 BaseBoard 内部 5 V 路径与保护器件、连接器接点载流、导体截面）推出一个**测试上限**与明确的**停止条件**（电流、pin17 压降、温升、时间各设阈值），在该上限以下记录整机开机、充电、屏幕满亮与音频满载时的电流与 pin17 压降。④ 上述测量只产出「在该上限以下可工作」的记录，**不产出额定值**；pin17 额定在官方给出数值或器件级依据前保持 `unknown`（Hiro H0 §5 c9） |
| A-H0-6 | `ASSUMPTION:` 实拍 `V1.2 / 260821` 与官方指南「1.2.1 及以上」为同一版次 | 无直接证据；BSP 只有 `V1_0`／`V1_2` 两档，**且 1.1 与 1.2 同映射 V1_2（Hiro 补查）**，故 variant 无分辨力 | 与 A-H0-3 同步登记，同样保存原始 `vX.Y` 与映射 `variant` 两个字段。**更正：原写「若三值一致且背面确无四焊盘，结论 1 可升 confirmed」不成立**——eFuse 无法分辨 1.2 与 1.2.1，实物背面无四焊盘只能确证「手上这块无」，要升 confirmed 还需官方给出 V1.2 的版本对照或 BaseBoard 资料。**（2026-09-21 按 Hiro H0 F02 整改）本条与四焊盘相关的统一表述收窄为：**「现有 V1.2／260821 图像未见 V1.0 中央四触点，官方适用 1.2.1＋的示意图亦未画出该组；两种版次口径关系未知。`ASSUMPTION:` 本次设计不依赖旧背部四触点，到货后核对目标样本。」不得由此推出「全部 V1.2 无四焊盘」或「背面确无 I²C 接口」 |
