...
以下两图是 **12 触点的方向示意**。浅蓝方块只是 Mosaico 假设外形，绿色为转接板，金色圆为板背面的占位触点；图中没有真实连接器、外壳、紧固件和导线。**圆点数量不代表已批准的针位或网名。**
...
| 总触点 | 信号 + 电源 + 地 | 判断 |
| --- | --- | --- |
| **12** | 10 + 1 + 1 | 十键直连、底座供电的**连通下限**。一个电源或地触点开路即失去供电；电流额定、压降与错位安全均未证实。 |
...
**不可据此直接定 12。** 现有错位分析用旧 4×4、旧针向和具体壳体包络算出 5V/GPIO、5V/GND 等风险；新的 2×6 图尚无电气网名，机械导向和错位公差也未给定。少地触点可能使 5V 更难隔离于 GPIO。必须针对新 pad 直径、针头、排布、容差和首次接触顺序重新做桥接／旋转／镜像检查；不能用「数量够连通」替代安全验证。供电还要核弹簧针、H2 pin17 `5V_IN`／pin20 GND、走线和保护通路的单点额定与温升。底座绝不向 pin18 `5V_OUT` 送电；并联底座触点也不能提高 H2 单针允许电流。
