...
**当前状态（2026-09-20）：CHANGES_REQUIRED。** Chrome 非作者复核（PR #12）复现了机制预期，同时复现两项采纳前必修问题 R01、R02，见下文「Chrome 非作者复核结论」。
...
> **接手主控补记（2026-09-20，claude-fable-5-1）：** 上一条补记写「这三个文件只存在于提案分支」时并不属实。
> PR #11（`fd64da1`）从 `main` 撤回实现时，没有把实现补进本分支：远端 `claude/t18-reproducible-baseline`
> 停在 `94b30d5`，只有 D-012～D-014 与 T18／T19 领取声明，没有实现，提案分支一度为空；实现只存在于
> 已撤回的 `main` 历史提交 `56030b9`（随 PR #10 合入）。Chrome 在 PR #12 的复核中也指出了这一点。
> 本提交由接手主控修复：把三个实现文件从 `56030b9` 原字节恢复到本分支，并把 `main` 上的本报告与
> `regenerated/` 对照产物一并带入，使提案自包含。恢复后逐个用 `sha256sum` 与 `git show 56030b9:<路径>`
> 比对，逐字节一致，且与 Chrome `review/chrome/T18/RECOVERY.json` 登记的哈希与字节数一致：
>
> | 文件 | SHA-256（与 `56030b9` 一致） | 字节 |
> | --- | --- | --- |
> | `design/build_design.py` | `418d7fe1ba99e744fa2bc73d63dbe141d03ef4bca8d75748dec4efa2fd4a3ea6` | 41696 |
> | `design/library-policy.json` | `b9f4ec8d258b6d1c70cbe2a1da8f3a297c37c56611f7423b7e2a26da1376e4b4` | 25745 |
> | `design/references-lock.json` | `520e8e48e39b469992c400a6c83e97a4492d3d7cfebfb649234faf3a240a218d` | 17036 |
>
> 本次恢复不修改实现内容，未修 R01／R02；第六节「未执行 `git add` / `commit` / `push`」是原提案撰写时的状态，
> 现已不适用于本分支。`design/` 下 5 份基线产物与 `references/` 下全部 31 份 LCSC 缓存（`C*.json`）在本分支上与 `main` 逐字节相同；
> 本分支基于 `94b30d5`，不含 `main` 后来新增的 `references/official-v12/`（与本提案无关，本任务未动 `references/`）。

## Chrome 非作者复核结论（PR #12）：CHANGES_REQUIRED

- 复核者：Chrome（GPT-6 Astra）。性质：对本提案的**非作者机制复核**，不是 Hiro 的独立硬件复核；未采纳 T18、未关闭 F03、未批准任何硬件。
- 报告位置：`origin/chrome/t18-proposal-review` 的 `review/chrome/T18/REPORT.md`（PR #12，合入后为 `main` 同路径）。同目录另有 `RECOVERY.json`（候选恢复记录）、`check_proposal.py`（隔离副本内的故障注入脚本）与 `check-results.json`（Python 3.9.6 下的结果）。
- 输入：`main` 为 `edb5301`；候选从 `56030b9` 原字节恢复到 `review/chrome/T18/candidate/`，未覆盖 `design/` 或 `references/`。
- 已复现的六项机制预期：无参运行退出码 2 且不覆盖基线；R26／C23239 未裁决时退出码 1 且无输出；`--allow-missing` 下两次普通运行与一次 `python3 -O` 五份产物逐字相同，并与本报告的 `regenerated/` 一致；删除已登记的 C45783 缓存即使给 `--allow-missing` 仍退出码 1；C25804 追加一个换行因哈希变化退出码 1；清单删掉 U3 条目即使给 `--allow-missing` 仍退出码 1。

同时复现两项**采纳前必须修复**的问题（以下为如实转述，位置行号以 Chrome 报告中对 `candidate/build_design.py` 的标注为准，与本分支恢复件同字节）：

### R01：`--write-policy` 重新登记会清空已填的人工审核字段（T18 新增行为）

- 位置：`build_design.py` 第 498–518 行（`policy_document()`）、第 535–543 行（`--write-policy` 分支）。
- 触发：在 R28 的清单条目填入 `reason`、`placeholder_geometry`、`review_state` 三个哨兵值，运行 `--write-policy`。命令退出码 0，前两项变为空串，`review_state` 变回「未审查」，已有裁决未被保留。原因：生成器按仓库现状重新推导整张骨架，未读取并合并已有清单。
- 本提案第五节末尾已承认该限制，Chrome 独立复现。
- 采纳前要求：默认保留已有条目的人工字段与 `policy`，只补新增位号；删除位号、编码变化、重置已有裁决必须显式处理；补「已有人工裁决再次登记不丢失」的回归检查。在此之前不得开始逐行填写正式清单。

### R02：缺少必需焊盘的缓存经 `--write-lock` 登记后仍可输出（继承自旧脚本，T18 应补）

- 位置：`build_design.py` 第 147–152 行（`resolve_code()` 只要求 `packageDetail` 非空即判 `OK`）、第 648–669 行（`footprint()` 只校验「已有 PAD 的脚号属于设计」）；`main` 旧脚本第 209–220 行同为单向检查。
- 触发：只在隔离副本中删除 C25804 封装内脚号 `2` 的一个 PAD，其余结构保留；先运行 `--write-lock` 如实登记修改后缓存的真实哈希，再运行 `--out-dir out --allow-missing`。两步均退出码 0。生成的 R1～R17 共 **17 个电阻**都只剩脚号 `1`，而各自的设计引脚集合仍为 `1、2`；BOM 状态全部仍为 `Supplier library geometry; final manufacturer audit pending`，没有 `UNRESOLVED` 标记。
- 性质：此例没有绕过哈希检查——锁文件登记的就是受损缓存的真实哈希；`--allow-missing` 只放行了 R26，受损的 C25804 既未被识别为问题，也未被降级放行。缺口在于「锁定字节」不等于「结构有效」：未验证全部应有物理引脚均有 PAD。该缺口继承自旧脚本，不是 T18 新引入的回归，但 T18 要建立「无效缓存硬失败」的基线，采纳前应补齐。
- 采纳前要求：按明确的物理引脚集合检查 PAD 覆盖，遗漏与额外脚号均阻断；同一脚号对应多个焊盘应允许；NC 电气脚仍需物理焊盘，纯机械焊盘例外必须显式声明；补「已重新登记哈希的缺 PAD 缓存仍失败」的回归检查。该检查不替代原厂封装核对。

### 对本提案状态的影响

- **采纳前必须先修 R01、R02 并补上述两项回归检查。** 本分支当前提交只做恢复，未修 R01／R02。
- Chrome 另列出待其自身裁决的事项（R26／C23239、6 份缓存冻结点、C5／C7／C8 与其余变更几何、U8 封装名、清单填写）以及三项机制取舍（生成器／清单自身哈希变化仅提示；`reason`／`placeholder_geometry`／`review_state` 不作为生成闸门；草案生成与基线登记的条件）。这些与第四节 D1～D14 对应，归属不变。
- Chrome 未实施 T19／F01 修复、未解决 F02、未做 ERC／DRC／EDA 导入／原厂封装／实物验证。Hiro 复核与放行链继续有效。
